...
def main():
    ...
    ).to_csv(RESULTS_DIR / "pm25_lightgbm_screen_errors.csv", index=False)

    pd.DataFrame({"horizon": horizons, "skill": skill}).to_csv(
        RESULTS_DIR / "pm25_lightgbm_screen_skill.csv", index=False
    )

    pd.DataFrame({"horizon": horizons, "n_valid_windows": valid_counts}).to_csv(
        RESULTS_DIR / "pm25_lightgbm_screen_counts.csv", index=False
    )

    (RESULTS_DIR / "pm25_lightgbm_screen_hstar.txt").write_text(f"H*={h_star}\n", encoding="utf-8")
    ...
    plt.xlabel("Horizon")
    plt.ylabel("MAE")
    plt.title("PM2.5 LightGBM Screening Error vs Horizon")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "pm25_lightgbm_screen_error_vs_horizon.png", dpi=150)
    plt.close()
    ...
    plt.title("PM2.5 LightGBM Screening Skill vs Horizon")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "pm25_lightgbm_screen_skill_vs_horizon.png", dpi=150)
    plt.close()
