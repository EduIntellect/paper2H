...
def main():
    ...
    ).to_csv(RESULTS_DIR / "wind_errors.csv", index=False)

    pd.DataFrame({"horizon": horizons, "skill": skill}).to_csv(
        RESULTS_DIR / "wind_skill.csv", index=False
    )

    (RESULTS_DIR / "wind_hstar.txt").write_text(f"H*={h_star}\n", encoding="utf-8")
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "wind_error_vs_horizon.png", dpi=150)
    plt.close()
    ...
    plt.title("Wind Skill vs Horizon")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "wind_skill_vs_horizon.png", dpi=150)
    plt.close()
