...
def evaluate_rolling_origin_arima(series: pd.Series, horizons: list[int]):
    ...
    for h in horizons_arr:
        ...
        for origin in origins:
            ...
            try:
                ...
                model_pred = float(fitted.forecast(steps=h).iloc[-1])
            except Exception:
                continue
...
def main():
    ...
    horizons, baseline_mae, model_mae = evaluate_rolling_origin_arima(
        series=series,
        horizons=HORIZONS,
    )
    ...
    ).to_csv(RESULTS_DIR / "wind_arima_noncanonical_errors.csv", index=False)

    pd.DataFrame({"horizon": horizons, "skill": skill}).to_csv(
        RESULTS_DIR / "wind_arima_noncanonical_skill.csv", index=False
    )

    (RESULTS_DIR / "wind_arima_noncanonical_hstar.txt").write_text(
        (
            f"H*(relax)={h_relax}\n"
            f"H*(strict)={h_strict}\n"
    ...
    plt.plot(
        horizons,
        model_mae,
        label=f"Model (ARIMA order={ARIMA_ORDER})",
        linewidth=2,
    )
    plt.xlabel("Horizon")
    plt.ylabel("MAE")
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "wind_arima_noncanonical_error_vs_horizon.png", dpi=150)
    plt.close()
    ...
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "wind_arima_noncanonical_skill_vs_horizon.png", dpi=150)
    plt.close()
